...
impl MemoryRepo {
    pub fn new(
        lyrics: impl Iterator<Item = Lyric>,
        playlists: impl Iterator<Item = Playlist>,
    ) -> Self {
        Self {
            db: Arc::new(RwLock::new(HashMap::from_iter(
                lyrics
                    .map(lyric_to_tuple)
                    .chain(playlists.map(playlist_to_tuple)),
            ))),
        }
    }
...
mod tests {
    ...
    use lipl_core::{LiplRepo, LyricPost, PlaylistPost};

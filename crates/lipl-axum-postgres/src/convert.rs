...
pub fn to_lyric(row: Row) -> Result<Lyric> {
    Ok(Lyric {
        id: row.try_get::<&str, reexport::uuid::Uuid>(sql::column::ID)?.into(),
...
pub fn to_playlist(row: Row) -> Result<Playlist> {
    Ok(Playlist {
        id: row.try_get::<&str, reexport::uuid::Uuid>(sql::column::ID)?.into(),
        title: row.try_get::<&str, String>(sql::column::TITLE)?,
        members: row.try_get::<&str, Option<Vec<reexport::uuid::Uuid>>>(sql::column::MEMBERS)?.unwrap_or_default().map(Uuid::from),
    })
}

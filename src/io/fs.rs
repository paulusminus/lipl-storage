...
use crate::model::{parts_to_string, PathBufExt, Lyric, LiplError, LiplResult, Playlist, PlaylistPost, Uuid, UuidExt, YAML, TXT};
...
where P: AsRef<Path> {
    ...
    for entry in read_dir(&dir_path)? {
        ...
        else if file_path.is_file() && file_path.has_extension(YAML) {
            let uuid = file_path.to_uuid();
            playlist_hm.insert(
                uuid,

...
pub fn get_lyrics<P: AsRef<Path>>(path: P) -> Result<impl Iterator<Item=model::Lyric>, Error> {
    read_dir(path)
    .map(|list|
        get_fs_files(list, "txt")
        .filter_map(
            |path_buffer| 
                File::open(&path_buffer).and_then(|f| get_lyric(f, path_buffer.to_uuid())).ok()
        )
    )
}

pub fn get_lyric(reader: impl Read, id: Uuid) -> Result<model::Lyric, Error> {
...
pub fn parts_from_reader<R: Read>(reader: BufReader<R>) -> Result<(Option<String>, Vec<Vec<String>>), Error>
{

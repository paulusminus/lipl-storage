...
async fn list(pool: PoolState) -> Result<(StatusCode, Json<Vec<Summary>>), Error> {
    pool.get()
        .map_err(Error::from)
        .and_then(db::list)
...
async fn delete(pool: PoolState, Path(id): Path<lipl_types::Uuid>) -> Result<StatusCode, Error> {
    pool.get()
        .map_err(Error::from)
